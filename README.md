...
ここで、README.mdを選択すると、以下のようにどこに差分が入ったかがハイライトされているため一目で変更の確認ができる。

![](images/TT4.png)

3. 変更のコミット

この状態ではまだローカルの変更がリモートに反映されていない状況である。

差分の確認後に、README.mdファイルの右側の４つのアイコンのうち、+アイコンを押して変更をステージに上げる。

![](images/TT5.png)

変更をステージに上げた後、Commitメッセージボックスにどのような変更が入ったかを入力する。必ず何か文字列を入れる必要がある。

![](images/TT6.png)

Commit後に以下のボタンが出るため、クリックしてリモートとの同期が完了する。

![](images/TT7.png)

以上がファイル変更のコミット方法である。

今回はREADME.mdをローカルで編集してそれをリモートにコミットしたが、アップロードしたいファイルをクローンによって作成されたフォルダにドラッグ&ドロップして配置し、同様にコミットすることでファイルアップロードが完了する。

## リモートの変更のローカルへの反映

一人で作業を行なっている場合はこの操作は不要であるが、最新のリポジトリをローカルにクローンした後に誰かがそのリポジトリに変更を加えた場合、ローカルのファイルはリモートのファイルよりも状態が遅れていることになる。もちろん、都度クローンし直せば最新のリポジトリを引っ張ってくることができるが、ローカルの状態をリモートと同様に最新にする方法がある。

この方法をPULLという。

以下のコマンドをターミナルで実行することで、リモートの最新版をローカルに引っ張ってくることができる。
